...
for i in range(10):
    ...
    decClass.append(model_range)
    preds_range = model_range.predict(te_rangeD)


preds = list()
for i in range(len(teData)):
    dec_pred = model_bydec.predict([teData[i]])

    # then go into decade model
    dec = int( dec_pred / 10 ) * 10
    ind = int ((dec - 1920) / 10)
    ...
    mod = decClass[ind]
    preds.append(mod.predict([teData[i]]))
    
figure1 = plt.figure()
title = 'Accuracy by Year (Multi-Regression), +/- a Year'
plt.title(title)
plot_by_year(teTarg, preds, 1)
plt.show()
